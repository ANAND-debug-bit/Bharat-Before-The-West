</head>
<body>

<h1>PROJECT TITLE: Bharat Before The West"</h1>

<hr>

<h3>AUTHOR : ATHARV ANAND</h3>

<br>

<h2>How To Use The Website</h2>

<p>
  It's very easy just open the website and you will see website headpage with a background image of Konark Sun Temple
  <br>
  ![image](https://www.image2url.com/r2/default/images/1781111019401-cb6f7b49-588d-414c-9d82-f83d0e9bb4ea.png)
<br>
  scroll down the website and experience all the event cards , different discoveries , different categories and at the end of the website with a disclaimer and source credits
  <br>
  ![image](https://www.image2url.com/r2/default/images/1781111164775-2ca1c20b-add3-437b-8282-913c489d0e82.png)

</p>
<hr>

<h3>Why I made this website ?</h3>

<hr>

<p>
  I made this website because I found that the world always give credits to the western scientists for the discoveries like gravity , medical surgeries etc but it was already way back discovered by Indians but they were never given the credits , so to dedicate their marvelleous discoveries I built this website.
</p>

<h2>Some cool features</h2>

<ul>
  <li> Both Indian and western claims stated per event card</li>
  <li> Background image of Konark Sun Temple at home page to represent an Indian styled website theme</li>
  <li> Classic Indian theme represented through text styling and events represetation<li>
  <li> Time gap between Discovery by an Indian and a western claim</li>
  <li>Separate pages or cards for each field — medicine, astronomy etc.</li>
  <li> Sources and disclaimer (to not hurt anyone's feelings) at the end of the website </li>
  <li> Smooth fade in transition during scroll down in the site </li>
</ul>

<hr>

<h3> AI use </h3>
<p>I used AI for all the taglines , research work for the discoveries done by Indians and what were the western claims and their time gap and also to remove a bug found during the execution of javascript file(it was just a function error) </p>
<hr>
<h2>TECH STACK</h2>

<br>


 <img width="80" height="28" alt="image" src="https://github.com/user-attachments/assets/249b14d7-9f15-47f4-ba10-f5849e89bef8" />

  <img width="49" height="28" alt="image" src="https://github.com/user-attachments/assets/03ae4f39-4206-4633-8bae-f2b3487167d5" />
</li>
  <img width="127" height="28" alt="image" src="https://github.com/user-attachments/assets/e29db953-058d-49c7-8401-8ea0718ce4b8" />

</li>
</ul>
</body>
</html>



